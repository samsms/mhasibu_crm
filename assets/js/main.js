...
$(function() {
    ...
    table_leads_reminder = $('table.table-leads_reminder');
    if (table_leads_reminder.length) {
        var tableLeadsConsentHeading = table_leads_reminder.find('#th-consent');
        var leadsTableNotSortable = [0];
        var leadsTableNotSearchable = [0, table_leads_reminder.find('#th-assigned').index()];
        ...
        _table_api = initDataTable(table_leads_reminder, admin_url + 'leads_reminder/table', leadsTableNotSearchable, leadsTableNotSortable, LeadsServerParams, [table_leads_reminder.find('th.date-created').index(), 'desc']);

        if (_table_api && tableLeadsConsentHeading.length > 0) {
            _table_api.on('draw', function() {
                var tableData = table_leads_reminder.find('tbody tr');
        ...
        $.each(LeadsServerParams, function(i, obj) {
            $('select' + obj).on('change', function() {
                ...
                $("[name='view_status[]']")
                    ...
                    table_leads_reminder.DataTable().ajax.reload()
                    .columns.adjust()
                    .responsive.recalc();
            });
        });
...
function lead_reminder_form_handler(form) {

    tinymce.triggerSave();

    // Disable the save button in cases od duplicate clicks
    $('#_reminder_modal').find('button[type="submit"]').prop('disabled', true);
...
function new_leads_reminder() {
    var url = admin_url + 'leads_reminder/reminder'; 
    ...
    var $taskSingleModal = $('#lead_reminder-modal');
    if ($taskSingleModal.is(':visible')) { $taskSingleModal.modal('hide'); }

    var $taskEditModal = $('#_reminder_modal');
    if ($taskEditModal.is(':visible')) { $taskEditModal.modal('hide'); }

    requestGet(url).done(function(response) {  
       $('#__reminder').html(response);
        $("body").find('#_reminder_modal').modal({ show: true, backdrop: 'static' });
    }).fail(function(error) {
        alert_float('danger', error.responseText);
    })
}
